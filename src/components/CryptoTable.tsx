...
const CryptoTable: React.FC<CryptoTableProps> = ({ data }) => {
  ...
  const sortedData = useMemo(() => {
    return data.slice().sort((a, b) => {
      if (sortType === "name") {
        const nameA = a.currency.name.toLowerCase();
        const nameB = b.currency.name.toLowerCase();
        if (sortByName === "asc") {
  ...
  }, [data, sortDirection, sortType, sortByName]);
  ...
  const handleSortChange = (
    type: "price" | "volumen" | "marketCap" | "name",
  ) => {
    setSortType(type);
  };
          ...
          <tr className="h-8 w-full border-b-2  bg-gray-200 p-4 ">
            ...
            <th
              onClick={() => {
                ...
                handleChangeSortDirectionByName();
              }}
